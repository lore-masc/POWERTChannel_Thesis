...
public class Task extends AsyncTask<Void, String, Void> {
    ...
    private static float bit_threshold_start = 0.50f;
...
//        bit_threshold_start = bit_threshold_start - overhead_workload;
    }
    ...
    protected Void doInBackground(Void... voids) {
        ...
        ArrayList<Float> workloads = new ArrayList<>();

        while (this.isRecording()) {
            Log.d("SINK", "" + bits);
            float measured_workload;

            // waiting first peak
            do {
                measured_workload = Math.max(0, Utils.readCore(READ_CORES) - overhead_workload);
                if (start && measured_workload >= PEAK) {
                    Log.d("SINK", "MEASURED: " + measured_workload);
                    start = false;
                    start_range = System.currentTimeMillis();
                }
            } while (start && measured_workload < PEAK);
            workloads.add(measured_workload);

            long last_measure = System.currentTimeMillis();

            if (workloads.size() > average_size) {
                workloads.remove(0);
                float workload = weightedAverageArray(workloads);
                publishProgress(mode, String.valueOf(workload), "", "");

                // synchronization
                if (workload >= bit_threshold_start && zero_bit) {
                    ...
                    zero_bit = false;
                } else if (workload < bit_threshold_start && !zero_bit) {
                    consecutive_bits = Math.max(1, Math.round((last_measure - start_range) / (wait * 1.0f)));
                ...
                if (!synchronization && bits.size() >= BYTE) {
                    ...
                    for (int i = 0; i < BYTE; i++)
                        bits.remove(0);
                    publishProgress(mode, String.valueOf(workload), String.valueOf(dec), "");
