/**
 * Modified version of Librosa ChiaChun FU library. In this version the ref_value is implemented in
 * powerToDb method considering the np.max() function.
 */

package com.powert.speechcommandapp.utility.mfcc;
...
public class MFCC {
    ...
    public double[][] powerToDb(double[][] melS){
        ...
        // ref is enabled.
        // ref_value calculation
        double ref_value = melS[0][0];
        for (int i = 0; i < melS.length; i++)
            for (int j = 0; j < melS[i].length; j++)
                ref_value = Math.max(melS[i][j], ref_value);
        ...
        for (int i = 0; i < melS.length; i++){
            for (int j = 0; j < melS[0].length; j++){
                if (ref_value > 1e-10){
                    log_spec[i][j] -= 10.0*log10(ref_value);
                }else{
                    log_spec[i][j] -= 10.0*(-10);
                }
                if (log_spec[i][j] > maxValue){
